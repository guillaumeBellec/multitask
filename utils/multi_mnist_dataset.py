...
class MultiMNIST(data.Dataset):
    ...
    def __init__(self,
        ...
        if multi:
            if self._train:
                ...
                self.train_data = self.prepare_images(self.train_data)
            else:
                self.test_data, self.test_labels_l, self.test_labels_r = torch.load(
                    os.path.join(self._root, self.processed_folder,
                                 self.multi_test_file))
                self.test_data = self.prepare_images(self.test_data)
        else:
            NotImplementedError()
            if self._train:
    ...
    def prepare_images(self, images):
        images = torch.stack([self._transform(img.reshape([1,28,28]).float()) for img in images])
        return images
...
def read_label_file(path, extension):
    with open(path, 'rb') as f:
        ...
        multi_labels_l = np.zeros((1 * length), dtype=np.int32)
        multi_labels_r = np.zeros((1 * length), dtype=np.int32)
...
if __name__ == '__main__':
    ...
    for dat in loader:
        ...
        for j in range(5):
            for i in range(2):
                axarr[i][j].pcolor(ims[j * 2 + i, :, :], cmap='gray_r')
